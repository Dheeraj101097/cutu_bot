...
import { GlassCard } from '../components/GlassCard';
import { GlassButton } from '../components/GlassButton';
// import { GlassInput } from '../components/GlassInput'; // unused but kept if you need it later
...
export function UploadScreen() {
  ...
  // NEW: Added a loading state to disable the button while generating
  ...
  // NEW: The function that talks to your Colab Python backend
  const handleGenerate = async () => {
    if (!prompt.trim()) return; // Don't send empty prompts

    setIsGenerating(true);
    try {
      // ⚠️ IMPORTANT: Replace this URL with your actual Ngrok public URL!
      // Example: 'https://1234-abcd.ngrok-free.app/generate'
      const API_URL = 'https://carmella-unbearing-overimaginatively.ngrok-free.dev/generate';
      ...
      // Update the bottom card with the new base64 image returned from Colab!
      setPreviewImage(data.image_url);
      
    } catch (error) {
      console.error("Error generating image:", error);
      alert("Failed to generate image. Make sure your Colab ngrok tunnel is running!");
    } finally {
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'upload' ? (
            ...
            >
              ...
              {/* UPDATED: GlassButton now triggers handleGenerate and shows loading state */}
              <GlassButton 
                onClick={handleGenerate}
                disabled={isGenerating || !prompt.trim()}
                className={`w-full py-4 flex items-center justify-center space-x-2 transition-colors ${
                ...
                <GlassCard className="w-64 p-3 relative group overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="aspect-square rounded-2xl overflow-hidden mb-4 border border-white/10 relative">
                      {/* Optional visual feedback on the image while generating */}
                      {isGenerating && (
